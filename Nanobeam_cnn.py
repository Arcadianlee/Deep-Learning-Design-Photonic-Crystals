...
for i in range(n_epochs):
    ...
    predError_train.append(pred_error(np.mean(train_outputArr[i*train_size:(i+1)*train_size-1]), np.mean(train_targetArr[i*train_size:(i+1)*train_size-1])))
    predErrorV_train.append(pred_error(np.mean(trainV_outputArr[i*train_size:(i+1)*train_size-1]), np.mean(trainV_targetArr[i*train_size:(i+1)*train_size-1])))

    t  = i+1
    predError_test.append(pred_error(np.mean(test_outputArr[t*test_size:(t+1)*test_size-1]), np.mean(test_targetArr[t*test_size:(t+1)*test_size-1])))
    predErrorV_test.append(pred_error(np.mean(testV_outputArr[t*test_size:(t+1)*test_size-1]), np.mean(testV_targetArr[t*test_size:(t+1)*test_size-1])))
