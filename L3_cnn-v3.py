...
f = h5py.File('/Users/Renjee/Desktop/CUHK/NOEL/Deep learning proj/code/L3_dataset/Input.mat','r')
data = f['Input']
Input = np.array(data) # For converting to a NumPy array

f = h5py.File('/Users/Renjee/Desktop/CUHK/NOEL/Deep learning proj/code/L3_dataset/Output.mat','r')
data = f['QnV']
...
def test():
    ...
    #save the model    
    if test_loss <= 0.0004:
        torch.save(network.state_dict(), '/Users/Renjee/Desktop/CUHK/NOEL/Deep learning proj/code/L3_model2.pt')
...
def movingAverage(interval, window_size):
    """ Moving average filter (data smoothener) """
...
train_outputArr = train_output.numpy() #for Q
train_targetArr = train_target.numpy()

trainV_outputArr = trainV_output.numpy() #for V
trainV_targetArr = trainV_target.numpy()

test_outputArr = test_output.numpy()
test_targetArr = test_target.numpy()

testV_outputArr = testV_output.numpy()
testV_targetArr = testV_target.numpy()
...
coeff_train = pearson_r(train_outputArr[len(train_outputArr)-10000:-1], train_targetArr[len(train_targetArr)-10000:-1])
coeff_test = pearson_r(test_outputArr[len(test_outputArr)-2500:-1], test_targetArr[len(test_targetArr)-2500:-1])
print('Training Q corr coeff: {:.3f}\tTest Q corr coeff: {:.3f}'.format(coeff_train, coeff_test))

coeffV_train = pearson_r(trainV_outputArr[len(trainV_outputArr)-10000:-1], trainV_targetArr[len(trainV_targetArr)-10000:-1])
coeffV_test = pearson_r(testV_outputArr[len(testV_outputArr)-2500:-1], testV_targetArr[len(testV_targetArr)-2500:-1])
...
for i in range(n_epochs):
    ...
    predError_train.append(pred_error(np.mean(train_outputArr[i*10000:(i+1)*10000-1]), np.mean(train_targetArr[i*10000:(i+1)*10000-1])))
    predErrorV_train.append(pred_error(np.mean(trainV_outputArr[i*10000:(i+1)*10000-1]), np.mean(trainV_targetArr[i*10000:(i+1)*10000-1])))

    t  = i+1
    predError_test.append(pred_error(np.mean(test_outputArr[t*2500:(t+1)*2500-1]), np.mean(test_targetArr[t*2500:(t+1)*2500-1])))
    predErrorV_test.append(pred_error(np.mean(testV_outputArr[t*2500:(t+1)*2500-1]), np.mean(testV_targetArr[t*2500:(t+1)*2500-1])))
...
fig = plt.figure()
plt.plot([x / 10000 for x in train_counter],train_losses,color='blue')
plt.plot([x / 10000 for x in test_counter],test_losses, linewidth = 2,color = 'red')
...
plt.scatter(10**(train_targetArr[len(train_targetArr)-10000:-1]),10**(train_outputArr[len(train_outputArr)-10000:-1]), s=5, color = 'purple')
...
fig = plt.figure()
plt.ticklabel_format(axis='both', style='sci', scilimits=(0,0))
plt.scatter(10**(test_targetArr[len(test_targetArr)-2500:-1]),10**(test_outputArr[len(test_outputArr)-2500:-1]), s=5, color = 'purple')
...
fig = plt.figure()
plt.ticklabel_format(axis='both', style='sci', scilimits=(0,0))
plt.scatter((trainV_targetArr[len(trainV_targetArr)-10000:-1]),(trainV_outputArr[len(trainV_outputArr)-10000:-1]), s=5, color = 'purple')
...
fig = plt.figure()
plt.ticklabel_format(axis='both', style='sci', scilimits=(0,0))
plt.scatter((testV_targetArr[len(testV_targetArr)-2500:-1]),(testV_outputArr[len(testV_outputArr)-2500:-1]), s=5, color = 'purple')
